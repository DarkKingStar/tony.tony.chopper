...
class Gogoanime extends models_1.AnimeParser {
    constructor() {
        ...
        this.fetchPopularAnime = async (page = 1,type = 1) => {
            try {
                const res = await this.client.get(`${this.baseUrl}/popular.html?page=${page}&type=${type}`);
                const $ = (0, cheerio_1.load)(res.data);
                const popularAnime = [];
                $('div.last_episodes > ul > li').each((i, el) => {
                    var _a, _b, _c, _d;
        ...
        this.fetchAnimeMovies = async (page = 1,type = 1) => {
            try {
                const res = await this.client.get(`${this.baseUrl}/anime-movies.html?page=${page}&type=${type}`);
                const $ = (0, cheerio_1.load)(res.data);
                const animeMovies = [];
                $('div.last_episodes > ul > li').each((i, el) => {
                    var _a, _b, _c, _d;
